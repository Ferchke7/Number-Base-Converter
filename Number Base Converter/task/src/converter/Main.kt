...
fun convertToDecimal(number: String, base: Int): BigDecimal {
    ...
    var powerCounter = number.split(".").first().length - 1

    number.replace(".", "").forEach { character ->
        result += digits.indexOf(character).toBigDecimal() * base.toDouble().pow(powerCounter).toBigDecimal()
        powerCounter--
    }
...
fun convertFromDecimal(number: BigDecimal, base: Int, isFractional: Boolean = true): String {
    ...
    val nonFractionalPartString = nonFractional.map { digits[it] }.joinToString("").ifEmpty { "0" }
